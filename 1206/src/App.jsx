...
function App() {
  const [selectDemo, setSelectDemo] = useState(1);
  const renderDemo=()=>{
    switch(selectDemo){
      case 1:
        ...
        <div className='grid grid-cols-4'>
          <div className='border-r'>
            <div className='sidebar-list'>
              {demolar.map((demo)=>(
                <button key={demo.id}
                onClick={()=>setSelectDemo(demo.id)}
                className={selectDemo===demo.id ? "bg-blue-500 text-white": "bg-gray-100"}>{demo.ad}</button>
              ))}
            </div>
          </div>
